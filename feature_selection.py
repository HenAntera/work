...
#lol = data[{'Volume','market_cap','total_volume','Value - Hrate','Value - Uadresses',
#            'volume_fi','momentum_mfi','volume_sma_em','volatility_bbm','volatility_kcp',
 #           'volatility_dch','trend_macd_diff','trend_sma_fast','trend_ema_fast','trend_adx','trend_adx_pos',
  #          'trend_adx_neg','trend_vortex_ind_neg','trend_mass_index','trend_ichimoku_base',
   #         'momentum_stoch','volume_nvi','volatility_atr',
    #        'volatility_bbp','volatility_bbli','trend_ema_slow','trend_vortex_ind_pos','trend_vortex_ind_diff',
...
#sss = make_pipeline(StandardScaler(),
 #                   VarianceThreshold(threshold=(0.8 * (1 - .8))),
  #                  GenericUnivariateSelect(f_classif, mode='percentile', param=0.8),
   #                 AdaBoostClassifier(base_estimator=GradientBoostingClassifier(), algorithm="SAMME", n_estimators=5, random_state=0, learning_rate=0.1)
    #                )
'''VARIANCE'''
...
sss = SVC(C=5, degree=1, kernel="poly")
...
sfs = SFS(AdaBoostClassifier(base_estimator=RidgeClassifierCV(),algorithm="SAMME",n_estimators=40, random_state=0, learning_rate=0.15), k_features=29, forward=True, 
          floating=False, scoring = 'accuracy', verbose=2, cv = splits, n_jobs=-1)

#[0, 3, 6, 8, 10, 13, 14, 24, 30, 32, 36, 38, 39, 40, 44, 45, 46, 48, 49, 50, 
#51, 52, 53, 54, 56, 61, 65, 66, 71, 72, 73, 78] 0.5652542372881355
#[0, 8, 9, 21, 28, 50, 74] 0.5838983050847457
